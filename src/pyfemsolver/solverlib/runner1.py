...
for pcenter, boundary in zip(nodes, is_boundary):
    if not (
        (np.abs(pcenter[0] - center_x[0]) < width / 2 - safety and np.abs(pcenter[1] - center_y[0]) < height / 2 - safety)
        or (np.abs(pcenter[0] - center_x[1]) < width / 2 - safety and np.abs(pcenter[1] - center_y[1]) < height / 2 - safety)
    ):
...
for i, trig in enumerate(tri2.simplices):
    for edge in [[0, 1], [1, 2], [2, 0]]:
        ...
        pcenter = 0.5 * (p0 + p1)
        if (np.abs(pcenter[0] - center_x[0]) < width / 2 - safety and np.abs(pcenter[1] - center_y[0]) < height / 2 - safety) or (
            np.abs(pcenter[0] - center_x[1]) < width / 2 - safety and np.abs(pcenter[1] - center_y[1]) < height / 2 - safety
        ):
            to_remove.append(i)
